...
#define RoundUp_Div(x, y)   ((int)(x/y) + ((x%y) ? 1:0))
#define SAT_CLUSTER_SIZE    0x40	// default Saturn block size (64 bytes)

u16 calculateUsedBlocks(int dataSize, int blockSize)
{
	int res, tmp;

	blockSize -= 4;
	res = RoundUp_Div(dataSize, blockSize);
	tmp = res * (blockSize-2) - dataSize - 30;
	if (tmp < 0)
		res += RoundUp_Div(-tmp, blockSize);
